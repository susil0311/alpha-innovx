...
import { Activity, Languages, Play, RotateCcw, ShieldAlert, SlidersHorizontal } from "lucide-react";
...
export function SimulationPanel({ onApply }: { onApply: (input: SimulationInput) => void }) {
  ...
  return <section className="mb-5 rounded-2xl border border-[#b9ccc9] bg-white p-4 shadow-panel lg:p-5">
    <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
      ...
      <div className="flex gap-2"><button onClick={reset} className="flex items-center gap-1.5 rounded-lg border border-[#d8e1e8] px-2.5 py-2 text-[10px] font-bold text-[#607285]"><RotateCcw className="size-3.5" /> Reset</button><button onClick={() => onApply(input)} className="flex items-center gap-1.5 rounded-lg bg-[#0b1f33] px-3 py-2 text-[10px] font-bold text-white"><Play className="size-3.5" /> Apply to map</button></div>
    </div>
    <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
      ...
      <div className="rounded-xl border border-[#d8e1e8] bg-[#f7fafb] p-4"><div className="mb-3 flex items-center justify-between"><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.14em] text-[#8493a0]"><Activity className="size-3.5 text-[#0f766e]" /> Calculated result</div><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${stateStyles[prediction.state]}`}>{prediction.state}</span></div><div className="flex items-end gap-4"><div><div className="text-4xl font-bold tabular text-[#0b1f33]">{Math.round(prediction.probability * 100)}%</div><div className="text-[10px] text-[#607285]">flash-flood probability</div></div><div><div className="text-2xl font-bold tabular text-[#c25f1c]">{prediction.leadTimeMinutes} min</div><div className="text-[10px] text-[#607285]">actionable lead time</div></div></div><div className="mt-4 space-y-2">{prediction.factors.map(factor => <div key={factor.label}><div className="mb-1 flex justify-between text-[10px]"><span>{factor.label}</span><b>{Math.round(factor.value * 100)}%</b></div><div className="h-1.5 rounded-full bg-[#e5ebef]"><div className="h-1.5 rounded-full bg-[#0f766e]" style={{ width: `${factor.value * 100}%` }} /></div></div>)}</div><div className="mt-4 flex items-start gap-2 rounded-lg bg-white p-3 text-[10px] leading-4 text-[#607285]"><ShieldAlert className="mt-0.5 size-3.5 shrink-0 text-[#c25f1c]" />{issueText}</div></div>
    </div>
    <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-[#e5ebef] pt-3 text-[10px] text-[#607285]"><span className="font-bold uppercase tracking-[.12em] text-[#8493a0]">Data quality</span><b className="text-[#0f766e]">{prediction.dataQuality}%</b><span>· confidence is reduced when sensors are stale or degraded</span><button onClick={() => onApply(input)} className="ml-auto flex items-center gap-1 font-bold text-[#0f766e]"><Languages className="size-3.5" /> Preview bilingual alert</button></div>
  </section>;
}
